...
void update() {
    ...
    if ( init_laser && init_robot ) {
        ...
        if ( !current_robot_moving ) {
            ...
            if (previous_robot_moving) store_background();
            else detect_motion();
        }
        else
        {
        ...
        if (!tracking_mode) detect_a_moving_person();
        else track_a_moving_person();
...
void detect_motion() {
    ...
    for (int loop=0; loop<nb_beams; loop++ )
      {//loop over all the hits
        if (background[loop] - r[loop] > detection_threshold) dynamic[loop] = true;//the current hit is dynamic
        else dynamic[loop] = false; //else its static
...
void track_a_moving_person() {
    ...
    if ( associated )
    {
        ...
        pub_datmo.publish(moving_person_tracked);
        if (frequency <= frequency_max) frequency++;
        uncertainty = uncertainty_min;
        ...
        ROS_INFO("moving_person_tracked: (%f, %f), %i, %f", moving_person_tracked.x,
                                                            ...
                                                            frequency,
                                                            uncertainty);
	if (uncertainty <= uncertainty_max) uncertainty += uncertainty_inc;
	frequency--;
